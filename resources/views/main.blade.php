...
<script>
    window.addEventListener('load', function () {
        ...
        lang.querySelector('select').addEventListener('change', function () {
            let selectedLocale = this.value
            let currentUrl = window.location.href
            let newUrl = currentUrl.replace(/\/(en|uk|fr|es)\//, `/${selectedLocale}/`);
            if (!newUrl.includes(`/${selectedLocale}/`)) {
                newUrl = `/${selectedLocale}` + window.location.pathname + window.location.search;
            }
